<x-app-layout>
    ...
    <div class="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('admin.attendance.store') }}" class="igym-card grid gap-4 p-5 md:grid-cols-4">
            ...
            <select name="course_id" class="rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-950"><option value="">Gym access only</option>@foreach($courses as $course)<option value="{{ $course->id }}">{{ $course->title }} · {{ $course->starts_at->format('M d') }}</option>@endforeach</select>
        ...
        <x-table>
            <thead class="bg-slate-50 dark:bg-slate-800/60"><tr><th class="px-4 py-3 text-start">Member</th><th class="px-4 py-3 text-start">Course</th><th class="px-4 py-3 text-start">Method</th><th class="px-4 py-3 text-start">Time</th></tr></thead>
