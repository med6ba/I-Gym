<x-app-layout>
    ...
    <div class="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('admin.subscriptions.store') }}" class="igym-card grid gap-4 p-5 lg:grid-cols-4">
            ...
            <select name="payment_status" class="rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-950"><option value="paid">Paid</option><option value="unpaid">Unpaid</option></select>
            ...
            <input type="hidden" name="status" value="active"><x-button>Create Subscription</x-button>
        </form>
        @if($expiring->isNotEmpty())<x-alert type="warning">{{ $expiring->count() }} subscriptions expire within 7 days.</x-alert>@endif
        <x-table>
            <thead class="bg-slate-50 dark:bg-slate-800/60"><tr><th class="px-4 py-3 text-start">Member</th><th class="px-4 py-3 text-start">Plan</th><th class="px-4 py-3 text-start">Ends</th><th class="px-4 py-3 text-start">Status</th></tr></thead>
            <tbody class="divide-y divide-slate-200 dark:divide-slate-800">@foreach($subscriptions as $subscription)<tr><td class="px-4 py-3 font-bold">{{ $subscription->member->name }}</td><td class="px-4 py-3">{{ $subscription->plan_name }}</td><td class="px-4 py-3">{{ $subscription->ends_at->format('M d, Y') }}</td><td class="px-4 py-3"><x-badge :status="$subscription->status" /></td></tr>@endforeach</tbody>
        </x-table>
        {{ $subscriptions->links() }}
    </div>
</x-app-layout>
