@php
    ...
    $gridClass = match ($navCount) {
        ...
        3 => 'grid-cols-3',
        4 => 'grid-cols-4',
        default => 'grid-cols-4 sm:grid-cols-5',
    };
@endphp
...
<nav class="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/95 lg:hidden">
    <div class="grid {{ $gridClass }} gap-1 px-2 py-2.5">
        ...
        @if($showProfile)
            ...
            <a href="{{ route('profile.edit') }}" class="{{ $profileActive ? 'bg-amber-100 text-amber-800 shadow-sm dark:bg-amber-950/60 dark:text-amber-200' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200' }} rounded-xl px-2 py-2.5 text-center text-xs font-bold transition">
        ...
        @endunless
        <button type="button" x-data x-on:click="$dispatch('open-modal', 'confirm-logout')" class="rounded-xl px-2 py-2.5 text-center text-xs font-bold text-slate-500 transition hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-400">
