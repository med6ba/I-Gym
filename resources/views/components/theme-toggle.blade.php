...
<div class="relative" x-data="{ open: false, theme: localStorage.getItem('igym-theme') || @js($theme) }" x-on:igym-theme-changed.window="theme = $event.detail">
    ...
    <div x-cloak x-show="open" x-transition x-on:click.outside="open = false" class="absolute end-0 z-50 mt-2 w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-950/10 dark:border-slate-800 dark:bg-slate-900">
        @foreach($themes as $code => $option)
            <button
                ...
                class="group flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-start text-sm font-bold transition"
            >
                <span class="flex items-center gap-3">
                    <span class="igym-menu-icon"><x-icon name="{{ $option['icon'] }}" size="16" /></span>
                    {{ $option['label'] }}
                </span>
