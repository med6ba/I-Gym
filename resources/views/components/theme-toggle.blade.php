@php
    ...
    $themes = [
        'light' => ['label' => __('messages.light'), 'icon' => 'palette'],
        'dark' => ['label' => __('messages.dark'), 'icon' => 'palette'],
        'system' => ['label' => __('messages.system'), 'icon' => 'settings'],
    ];
@endphp
...
<div class="relative" x-data="{ open: false }">
    ...
    <div x-cloak x-show="open" x-transition x-on:click.outside="open = false" class="absolute end-0 z-50 mt-2 w-44 rounded-xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-950/10 dark:border-slate-800 dark:bg-slate-900">
        @foreach($themes as $code => $option)
            <button type="button" x-on:click="window.igymSetTheme('{{ $code }}'); open = false" class="{{ $theme === $code ? 'bg-amber-50 text-amber-800 dark:bg-amber-950/40 dark:text-amber-200' : 'text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800' }} flex w-full items-center gap-3 rounded-lg px-3 py-2 text-start text-sm font-bold transition">
